...
public class SymbolTable {
    ...
    public void define(String name,String type,int kind) {
        ...
        if(kind == Kind.STATIC){ classTable.put(name,new Tuple(name,type,kind,StaticID ++)); }
        else if(kind == Kind.FIELD){ classTable.put(name,new Tuple(name,type,kind,FieldID ++));}
        else if(kind == Kind.VAR){ subroutineTable.put(name,new Tuple(name,type,kind,VarID ++));}
        else if(kind == Kind.ARG){ subroutineTable.put(name,new Tuple(name,type,kind,ArgID ++));}
    }
